...
for T in 110.0:2.0:160.0
    ...
    # INITIAL PARAMETERS
    samples_matrix = [2π * 0.00000000002 * sin(2π * (t / n_samples)) for t in 0:n_samples, i in 1:n_qubits]
    ...
    # E_actual = weights[8] * Λ[1] + weights[7] * Λ[2] + weights[6] * Λ[3] + weights[5] * Λ[4] + weights[4] * Λ[5] + weights[3] * Λ[6] + weights[2] * Λ[7]+ weights[1] * Λ[8]
    ...
    function gradient_ode!(Grad::Vector{Float64}, samples::Vector{Float64};
        ...
        fs = 1 / (0.1)      # Sampling frequency (GHz)
        cutoff = fs / 2.00000001  # Cutoff frequency (GHz) 
    ...
    tol_ode = 1e-5
    @time energy2 = costfunction_o(samples_initial)
    println("initial energy ", energy2)
    tol_ode = 1e-6
    @time energy3 = costfunction_o(samples_initial)
    println("initial energy ", energy3)
