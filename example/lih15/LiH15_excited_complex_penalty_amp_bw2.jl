...
for T in 144.0:2.0:160.0
    ...
    tol_ode = 1e-6
    Λ, U = eigen(Cost_ham)
    weights = [0.0, 0.1, 0.15, 0.2, 0.25, 0.3, 0.95] # this is a good one except T=132
    # weights = [0.0, 0.1, 0.15, 0.2, 0.25, 0.3, 0.97] # this one works for T=132 
    ...
    function gradient_ode!(Grad::Vector{Float64}, samples::Vector{Float64};
        λ_amp=1.0, Ω₀=2π * 0.02,sample_rate=0.2)
