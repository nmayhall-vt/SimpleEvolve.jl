...
tol_ode=1e-6
Λ, U = eigen(Cost_ham)
E_actual = Λ[1]+Λ[2]+Λ[3]+Λ[4]+Λ[5]+Λ[6]+Λ[7]
...
τ = T/n_samples_grad
# weights=[0.0,0.01,0.02,0.03,0.04,0.1,1.0] #this works quite good
weights=[0.0,0.01,0.03,0.06,0.1,0.15,1.0]
...
function gradient_ode!(Grad::Vector{Float64}, samples::Vector{Float64};
    ...
    grad_final = copy(grad_real)  

    if penalty
        for k in 1:n_qubits, i in 1:n_samples+1
            ...
            grad_final[i, k] += λ * grad_penalty
        end
    end

    # === Flatten back to Grad ===
    Grad[1:n]       .= vec(grad_final)
    Grad[n+1:end]   .= vec(grad_imag)
...
function costfunction_o(samples::Vector{Float64})
    ...
    # Compute energy 
    if penalty==true
        energy, Ψ_ode, energies = SimpleEvolve.costfunction_ssvqe_with_penalty_real(
...
Ω_plots_final = plot(                       
    ...
    title = "Final img amps",
    legend = false,
    layout = (n_qubits,1),
)
plot(Ω_plots, Ω_plots_final, layout=(1,2))
savefig("final_signals_$(n_qubits)_$(n_levels)_$(SYSTEM)_$(n_samples)_$(T)_penalty_on_real_amps.pdf")
...
energies=A.values
@save "samples_final_$(T)_$(SYSTEM)_penalty_on_real_amps_only.jld2" samples_final energies
