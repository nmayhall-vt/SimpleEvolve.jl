...
# for T in 150.0:5.0:300.0
T = 300.0
    println("T = $T")
    Cost_ham = npzread("qubit_op_H4_linear0.80.npy")
    # Cost_ham = npzread("qubit_op_H4_rec0.90.npy")
    # display(Cost_ham)
    n_qubits = round(Int, log2(size(Cost_ham, 1)))
    n_levels = 2
    SYSTEM = "h4_LINEAR"
    # SYSTEM = "h4_RECTANGULAR"
    ...
    # initial_state_ground = "110000"
    initial_state_ground = "000011"
    ψ_initial_g = zeros(ComplexF64, n_levels^n_qubits)
    ψ_initial_g[1+parse(Int, initial_state_ground, base=n_levels)] = 1.0 + 0im
    # ψ_initial_g = npzread("../REF_8_4_2_P-m.npy")
    # excited_configs = ["101000", "100100"]## "100010"]#"100001",011000", "010100", "010010", "001100", "001010", "000110"
    excited_configs = ["000101", "001001" ,"010100", "010010", "001100"]#, "001010", "000110"]
    ψ_initial_excited = [zeros(ComplexF64, n_levels^n_qubits)
    ...
    tol_ode = 1e-6
    Λ, U = eigen(Cost_ham)
    weights = [0.0, 0.02,0.04,0.08,0.16,0.9]
    println("Weights: ", weights)
    weighted = true
    E_actual = weights[6] * Λ[1] + weights[5] * Λ[2] + weights[4] * Λ[3] + weights[3] * Λ[4]+
        weights[2] * Λ[5] + weights[1] * Λ[6]
    ...
    B = [Λ[1]
        ...
        Λ[5]
        Λ[6]]
    display(A.values[1:n_states] .- B[1:n_states])
    energies = A.values
    # @save "samples_final_$(T)_$(SYSTEM)_excited.jld2" samples_final energies
# end